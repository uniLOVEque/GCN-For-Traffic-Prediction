...
- **历史交通数据** \( X_G^{t-P+1}, \dots, X_G^t \)：反映过去 P 个时间步在 N 个传感器节点的观测值（如车速、车流量）；
- **外部因素** \( F_1^{(t)}, F_2^{(t)}, \dots, F_n^{(t)} \)：包括天气、节假日、大型事件等外生信息。
...
- 输入：历史交通数据张量 \( X \in \mathbb{R}^{B \times C \times N \times T} \)
...
$$
\hat{Y} = f(FM_{fused}) \in \mathbb{R}^{B \times C' \times N \times Q}
$$
